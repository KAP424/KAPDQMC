function ctrl_SCEEicr(path::String, model::tUV_Hubbard_Para_, indexA::Vector{Int64}, indexB::Vector{Int64}, Sweeps::Int64, λ::Float64, Nλ::Int64, ss::Vector{Array{UInt8,2}}, record)
    ...
    SCEE = SCEEBuffer(model.Type, model.Ns)
    A = AreaBuffer(model.Type, indexA)
    B = AreaBuffer(model.Type, indexB)
    ...
    for loop in 1:Sweeps
        ...
        for lt in 1:model.Nt
            ...
            for i in axes(ss[1], 1)
                x, y = model.nnidx[i]
                tmpN[x] += model.η[ss[1][i, lt]]
                tmpN[y] += model.a * model.η[ss[1][i, lt]]
                tmpN_[x] += model.η[ss[2][i, lt]]
                tmpN_[y] += model.a * model.η[ss[2][i, lt]]
            end
...
function UpdateSCEELayer!(rng, s1, s2, G1::G4Buffer_, G2::G4Buffer_, A::AreaBuffer_, B::AreaBuffer_, model::tUV_Hubbard_Para_, UPD::UpdateBuffer_, SCEE::SCEEBuffer_, λ)
    for i in axes(s1, 1)
        x, y = model.nnidx[i]
        UPD.subidx = [x, y]
